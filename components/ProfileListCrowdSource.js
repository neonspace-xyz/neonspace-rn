...
const ProfileListCrowdSource = ({ tab, isProfile, usersession, userInfo }) => {
    ...
    <View style={[isProfile ? styles.containerListProfile : styles.containerList]}>
      <FlatList
        ...
        renderItem={({ item, index }) => {
          // console.log('c-datetime', index, item.id, item.datetime, item.item_type);
          if (item.item_type == 'hiring') {
            return (
              <CrowdSectionHiring
                tab={tab}
...
export default ProfileListCrowdSource;
